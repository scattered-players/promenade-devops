...
echo "s3fs#${short_short_name}-secret /mnt/secret fuse _netdev,rw,uid=1000,gid=1000,allow_other,iam_role=${short_short_name}-show-role" | sudo tee -a /etc/fstab

# sudo systemctl disable show;
sudo cp show.service /etc/systemd/system/show.service;
