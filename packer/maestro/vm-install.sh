...
echo "s3fs#${short_short_name}-secret /mnt/secret fuse _netdev,rw,uid=1000,gid=1000,allow_other,iam_role=${short_short_name}-maestro-role" | sudo tee -a /etc/fstab

# sudo systemctl disable maestro;
sudo cp maestro.service /etc/systemd/system/maestro.service;
